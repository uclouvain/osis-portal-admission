...
from admission.contrib.enums.pool_questions import ResidentConcoursDAcces
...
class PoolQuestionsForm(forms.Form):
    ...
    residence_certificate = FileUploadField(
        ...
        help_text=_('Official recent document issued by your local municipal administration certifying '
                    'that your main residence is located in Belgium.'),
        max_files=1,
        required=False,
    )
    resident_competitive_entrance_examination = forms.ChoiceField(
        label=_(
            'Have you taken a competitive entrance examination to study medicine, dentistry, pharmacy or '
            'physiotherapy (in Belgium or abroad), with the exception of the ARES medical or dental entrance '
            'examination?'
        ),
        widget=forms.RadioSelect(),

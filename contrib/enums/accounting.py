...
class ChoixAssimilation5(ChoiceEnum):
    A_NATIONALITE_UE = mark_safe(
        _('%(person_concerned)s has the nationality of a country of a Member State of the European Union')
        % {'person_concerned': dynamic_person_concerned}
    )
    TITULAIRE_TITRE_SEJOUR_LONGUE_DUREE = mark_safe(
        _('%(person_concerned)s has a long-term residence permit (B, C, D, F, F+ or M cards) in Belgium')
        % {'person_concerned': dynamic_person_concerned}
    )
    ...
    PRIS_EN_CHARGE_OU_DESIGNE_CPAS = mark_safe(
        _('%(person_concerned)s is supported by the CPAS, or by a CPAS rest home or designated by the CPAS')
        % {'person_concerned': dynamic_person_concerned}
    )
